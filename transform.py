...
def transform_db(df):
    ...
    # unify data
    folder_path = 'D:\\DS108\\Vietnamese_address_parsing\\entities'
...
def transform_address(data):
    ...
    path = 'D:\\DS108\\Vietnamese_address_parsing'
    dir_name = 'entities'
    address_dict = am.load_address_dict(path, dir_name)
...
if __name__ == '__main__':
    ...
    data = pd.read_excel('D:\\DS108\\source_code\\address_full_0712.xlsx')
    data.drop(columns=['Unnamed: 0'], inplace=True)
    raw_data = data.loc[101:, :]
    raw_data = raw_data.rename(columns={'Address': 'address'})
